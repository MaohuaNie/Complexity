...
transformed parameters {
	...
	for (n in 1:N) {
		...
		drift_t[n] = theta_sbj * (ub[n] - ua[n]+beta_sbj);
...
initFunc <-function (i) {
  ...
  for (ll in 1:i){
    initList[[ll]] = list(
                          ...
                          theta_v = runif(1,-0.001,0.001),
                          threshold_v = runif(1,-0.001,0.001),
                          ndt_v = runif(1,-1,-0.999),
                          rel_sp = runif(1,-0.6,-0.4)
    )
  }
  return(initList)
}
...
for (i in 1:n_iter) {
  ...
  dataList  = list(cho = stim5$simcho, accuracy_flipped = stim5$cho2, rt = stim5$simrt, N=60, 
                 oa = as.matrix(stim5[, c("payoffa1", "payoffa2")]),
                 ob = as.matrix(stim5[, c("payoffb1", "payoffb2")]),
                 pa = as.matrix(stim5[, c("proba1", "proba2")]),
                 pb = as.matrix(stim5[, c("probb1", "probb2")])
  ...
  samples <- extract(dsamples, pars = c('alpha_sbj', 'theta_sbj', 'threshold_sbj', 'ndt_sbj','transf_rel_sp'))
  ...
  sp_p_recover[i] <- mean(samples$transf_rel_sp)
  ...
  sp_p_bias[i] <- (mean(samples$transf_rel_sp)-sp_p)/sp_p
  ...
  sp_p_dev[i] <- abs(mean(samples$transf_rel_sp)-sp_p)/sp_p
...
df_summary <- data.frame(original_th_p = th_p, 
                 ...
                 original_sp_p = sp_p, 
                 recovered_sp_p = sp_p_recover,
                 bias_sp_p = sp_p_bias,
                 deviation_sp_p = sp_p_dev
...
df_mean <- data.frame(parameter = c('th_p', "theta_p", "ndt_p", "alpha_p",'sp_p'),
                      true_value = c(th_p, theta_p,ndt_p, alpha_p, sp_p),
                      mean_recovered = c(mean(df_summary$recovered_th_p), mean(df_summary$recovered_theta_p),mean(df_summary$recovered_ndt_p),mean(df_summary$recovered_alpha_p),mean(df_summary$recovered_sp_p)),
                      mean_bias = c(mean(df_summary$bias_th_p), mean(df_summary$bias_theta_p),mean(df_summary$bias_ndt_p),mean(df_summary$bias_alpha_p), mean(df_summary$bias_sp_p)),
                      mean_deviation = c(mean(df_summary$deviation_th_p), mean(df_summary$deviation_theta_p),mean(df_summary$deviation_ndt_p),mean(df_summary$deviation_alpha_p), mean(df_summary$deviation_sp_p))
                 )
df_mean
```
```{r}
df_median <- data.frame(parameter = c('th_p', "theta_p", "ndt_p", "alpha_p", 'sp_p'),
                      true_value = c(th_p, theta_p,ndt_p, alpha_p ,sp_p),
                      median_recovered = c(median(df_summary$recovered_th_p), median(df_summary$recovered_theta_p),median(df_summary$recovered_ndt_p),median(df_summary$recovered_alpha_p), median(df_summary$recovered_sp_p))
                      )
...
hist(df_summary$recovered_sp_p)
```
```{r}
