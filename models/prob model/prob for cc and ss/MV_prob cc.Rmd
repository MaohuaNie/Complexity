...
dat <- dat %>%
  mutate(
    trialtype = ifelse(skew == 'rl', 1, ifelse(skew == 'lr', -1, 0)), # 1 rl -1 lr 0 ns
    con = ifelse(test_part == 'cc', 1, -1),
  )
...
dataList  = list(cho = dat$cho, rt = dat$rt, participant = dat$tid,N=nrow(dat),  L = length(tids),starting_point=0.5, evd = dat$evd, sdd = dat$sdd, trialtype = dat$trialtype, con = dat$con)
...
```{r}
parameters = c("transf_mu_alpha","transf_mu_threshold","transf_mu_ndt", "transf_mu_theta",'transf_mu_delta', 'transf_mu_zeta', 'sd_threshold',"sd_alpha","sd_ndt", 'sd_theta', 'sd_delta', 'sd_zeta', "alpha_sbj","threshold_sbj","ndt_sbj",'theta_sbj', 'delta_sbj','zeta_sbj', "log_lik")
...
initFunc <-function (i) {
  ...
  for (ll in 1:i){
    initList[[ll]] = list(
                          ...
                          z_zeta = runif(length(tids),-0.1,0.1)
