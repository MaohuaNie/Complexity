...
dataList  = list(cho = dat$cho, rt = dat$rt, participant = dat$tid, N=nrow(dat),  L = length(tids), starting_point=0.5, evd = dat$evd, sdd = dat$sdd)
```


```{r message=FALSE, warning=FALSE}
m <- stan_model("MV_Baseline cc.stan")
dsamples <- sampling(m,  
                data=dataList, 
                pars=parameters,
                iter=2000, 
