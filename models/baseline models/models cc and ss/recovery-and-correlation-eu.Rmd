...
for (i in 1:n_iter) {
  ...
  for(n in 1:nrow(stim)){
    cres <- rwiener(1,th_p, ndt_p, sp_p, (theta_p + delta_theta_p * stim$con[n])  * ((pa[n,1] * (oa[n,1]^alpha_p) + pa[n,2] * (oa[n,2]^alpha_p)) - (pb[n,1] * (ob[n,1]^alpha_p) + pb[n,2] * (ob[n,2]^alpha_p))))
  ...
  samples <- rstan::extract(dsamples, pars = c("alpha_sbj","threshold_sbj","ndt_sbj",'theta_sbj', 'delta_theta'))
