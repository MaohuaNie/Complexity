```{r}
# 4 choices with same options just differ complex vs. simple
# 15 levels: 
# 4 control trials (EU Diff: 2 for 20 and -20, SD Diff 0 and no Skewness) for Easy vs. Easy and Compound vs. Compound, and Compound vs. Easy and Easy vs. Compound : 16 in total, 8 20 and 8 -20
...
for(level in ev_levels) {
  trials <- replicate(20000, generate_evs(level), simplify = FALSE)
...
df_no_duplicates <- na.omit(df_control)[!duplicated(na.omit(df_control)$EV_A), ]
...
df_no_dominant <- df_no_dominant[!(df_no_dominant$P_A1 %in% c(0.55, 0.45)), ]
df_no_dominant <- df_no_dominant[!(df_no_dominant$P_B1 %in% c(0.55, 0.45)), ]
df_no_dominant
```
...
solve_equations <- function(P_A1, P_A2) {
  ...
  while(TRUE) {
    # 1. Generate a random value for x1 between 0.2 and 0.8
    ...
    # Generate h1 between 0.2 and 0.8
    h1 <- round(runif(1, 0.21, 0.79), 2)
    if (h1 == x1 || h1 == y1 || h1 < 0.21 || h1 > 0.79) next
    
    # Calculate i1
    i1 <- round(1 - h1, 2)
    if (i1 == x1 || i1 == y1 || i1 == h1 || i1 < 0.21 || i1 > 0.79) next
    
    # Calculate j1 and k1 using the given equations
    j1 <- round((P_A1 - x1 * h1) / y1, 2)
    if (j1 == x1 || j1 == y1 || j1 == h1 || j1 == i1 || j1 < 0.21 || j1 > 0.79) next

    k1 <- round((P_A2 - x1 * i1) / y1, 2)
    if (k1 == x1 || k1 == y1 || k1 == h1 || k1 == i1 || k1 == j1 || k1 < 0.21 || k1 > 0.79) next
...
num_rows <- nrow(df_no_dominant)
for (row_index in 1:num_rows) {
  P_A1 <- df_no_dominant$P_A1[row_index]
  P_A2 <- df_no_dominant$P_A2[row_index]
  
  probabilities <- solve_equations(P_A1, P_A2)
  
  df_no_dominant$x1[row_index] <- probabilities$x1
  df_no_dominant$y1[row_index] <- probabilities$y1
  
  df_no_dominant$h1[row_index] <- probabilities$h1
  df_no_dominant$i1[row_index] <- probabilities$i1
  
  df_no_dominant$j1[row_index] <- probabilities$j1
  df_no_dominant$k1[row_index] <- probabilities$k1
}
...
solve_equations2 <- function(P_A1, P_A2) {
  ...
  while(TRUE) {
    ...
    # 2. Calculate y1 based on x1
    y2 <- round(1 - x2, 2)
    if (x2 == y2 || y2 < 0.21 || y2 > 0.79) next
    
    # Generate h1 between 0.02 and 0.98
    h2 <- round(runif(1, 0.21, 0.79), 2)
    if (h2 == x2 || h2 == y2 || h2 < 0.21 || h2 > 0.79) next
    
    # Calculate i1
    i2 <- round(1 - h2, 2)
    if (i2 == x2 || i2 == y2 || i2 == h2 || i2 < 0.21 || i2 > 0.79) next
    
    # Calculate j1 and k1 using the given equations
    j2 <- round((P_A1 - x2 * h2) / y2, 2)
    if (j2 == x2 || j2 == y2 || j2 == h2 || j2 == i2 || j2 < 0.21 || j2 > 0.79) next

    k2 <- round((P_A2 - x2 * i2) / y2, 2)
    if (k2 == x2 || k2 == y2 || k2 == h2 || k2 == i2 || k2 == j2 || k2 < 0.21 || k2 > 0.79) next
...
num_rows <- nrow(df_no_dominant)
for (row_index in 1:num_rows) {
  P_B1 <- df_no_dominant$P_B1[row_index]
  P_B2 <- df_no_dominant$P_B2[row_index]

  
  probabilities <- solve_equations2(P_B1, P_B2)
  
  df_no_dominant$x2[row_index] <- probabilities$x2
  df_no_dominant$y2[row_index] <- probabilities$y2
  df_no_dominant$h2[row_index] <- probabilities$h2
  df_no_dominant$i2[row_index] <- probabilities$i2
  df_no_dominant$j2[row_index] <- probabilities$j2
  df_no_dominant$k2[row_index] <- probabilities$k2
}
...
# Filtering the data to ensure x1 != x2, y1 != y2, and so on
df_no_dominant <- df_no_dominant[df_no_dominant$x1 != df_no_dominant$x2 & df_no_dominant$y1 != df_no_dominant$y2 & df_no_dominant$h1 != df_no_dominant$h2 & df_no_dominant$i1 != df_no_dominant$i2 & df_no_dominant$j1 != df_no_dominant$j2 & df_no_dominant$k1 != df_no_dominant$k2, ]

print(df_no_dominant)
...
df_no_dominant_int <- df_no_dominant
for(col in cols_to_convert) {
  df_no_dominant_int[[col]] <- as.integer(round(df_no_dominant_int[[col]] * 100))
}
...
row1 = round(runif(1, 10, 10))

eva = df_no_dominant[row1,]$O_A1*df_no_dominant[row1,]$P_A1 + df_no_dominant[row1,]$O_A2*df_no_dominant[row1,]$P_A2

evb = df_no_dominant[row1,]$O_B1*df_no_dominant[row1,]$P_B1 + df_no_dominant[row1,]$O_B2*df_no_dominant[row1,]$P_B2

eva1 = df_no_dominant[row1,]$O_A1*(df_no_dominant[row1,]$x1*df_no_dominant[row1,]$h1 + df_no_dominant[row1,]$y1*df_no_dominant[row1,]$j1) + df_no_dominant[row1,]$O_A2*(df_no_dominant[row1,]$x1*df_no_dominant[row1,]$i1 + df_no_dominant[row1,]$y1*df_no_dominant[row1,]$k1)

evb1 = df_no_dominant[row1,]$O_B1*(df_no_dominant[row1,]$x2*df_no_dominant[row1,]$h2 + df_no_dominant[row1,]$y2*df_no_dominant[row1,]$j2) +df_no_dominant[row1,]$O_B2*(df_no_dominant[row1,]$x2*df_no_dominant[row1,]$i2 + df_no_dominant[row1,]$y2*df_no_dominant[row1,]$k2)
...
```
```{r}
write.csv(df_no_dominant, "eens2/stimuli_control.csv", row.names = T)
write.csv(df_no_dominant_int, "eens2/stimuli_control_int.csv", row.names = T)
```

```{r}
# Select the first 8 rows
first_eight_rows <- df_no_dominant_int[1:8, ]

# Select the last 8 rows
last_eight_rows <- df_no_dominant_int[(nrow(df_no_dominant_int) - 7):nrow(df_no_dominant_int), ]

# Combine the first 8 and last 8 rows into one dataframe
combined_rows <- rbind(first_eight_rows, last_eight_rows)
combined_rows
```

```{r}
same_rows <- df_no_dominant_int[9:12, ]
same_rows
```


```{r}
write.csv(combined_rows, "eens2/stimuli_control_int.csv", row.names = T)
write.csv(same_rows, "eens2/stimuli_same_int.csv", row.names = T)
```

```{r}
# Generates row numbers for each kind of gamble
result_row_ee <- 1 + 4 * (0:3)  
result_row_ec <- 2 + 4 * (0:3)
result_row_ce <- 3 + 4 * (0:3)
result_row_cc <- 4 + 4 * (0:3)
...
# Create 'special' directory if it doesn't exist
if (!dir.exists("special")) {
  dir.create("special")
}
...
for (rownumber in result_row_ee) {
  trial <- combined_rows[rownumber,]
  values <- list(trial$P_A1, trial$P_A2, round(trial$O_A1), round(trial$O_A2))
  xml_content <- create_drawio_xml(values)
  
  # Save to the 'special' folder
  file_path <- sprintf("special/cn_trial%d_optionA.drawio", rownumber)
...
for (rownumber in result_row_ee) {
  ...
  # Save to the 'special' folder
  file_path <- sprintf("special/cn_trial%d_optionB.drawio", rownumber)
  writeLines(xml_content, con = file_path)
}
...
for (rownumber in 1:2) {
  trial <- same_rows[rownumber,]
  values <- list(trial$P_A1, trial$P_A2, round(trial$O_A1), round(trial$O_A2))
  xml_content <- create_drawio_xml(values)
  
  # Save to the 'special' folder
  file_path <- sprintf("special/same%d_optionA.drawio", rownumber)
  writeLines(xml_content, con = file_path)
}

for (rownumber in 3:4) {
  trial <- same_rows[rownumber,]
  values <- list(trial$P_A1, trial$P_A2, round(trial$O_A1), round(trial$O_A2))
  xml_content <- create_drawio_xml(values)
  
  # Save to the 'special' folder
  file_path <- sprintf("special/same%d_optionB.drawio", rownumber)
  writeLines(xml_content, con = file_path)
}
...
counter <- 1
for (i in same_index) {
  trial_data <- combined_rows[i, ]
  
  result[[counter]] <- sprintf("{ stimulus: 'stimu_same/cn_trial%s_optionA.png, stimu_same/cn_trial%s_optionB.png', data: { test_part: 'control'}, F: {probA1: %s, payA1: %s, probA2: %s, payA2: %s}, J: {probB1: %s, payB1: %s, probB2: %s, payB2: %s} }",
                         i,i,
...
for (i in diff_index) {
  ...
  result[[counter]] <- sprintf("{ stimulus: 'stimu_diff/cn_trial%s_optionA.png, stimu_diff/cn_trial%s_optionB.png', data: { test_part: 'control'}, F: {probA1: %s, payA1: %s, probA2: %s, payA2: %s}, J: {probB1: %s, payB1: %s, probB2: %s, payB2: %s} }",
                         i,i,
                         trial_data$P_A1, trial_data$O_A1, trial_data$P_A2, trial_data$O_A2,
                         trial_data$P_B1, trial_data$O_B1, trial_data$P_B2, trial_data$O_B2)
